...
export function signInMap(): string {
  const nodes = [{ x: 52, y: 252 }, { x: 122, y: 180 }, { x: 188, y: 226 }, { x: 252, y: 212 }, { x: 310, y: 158 }, { x: 282, y: 76 }];
  const route = "M52 252 C72 240 98 182 122 180 C148 178 162 230 188 226 C214 222 226 222 252 212 C278 202 312 186 310 158 C308 130 274 106 282 76";
  const path = `<path d="${route}" fill="none" stroke-width="6" stroke-dasharray="6 11" stroke-linejoin="round"/>`;
  const pins = nodes.map((point, i) => `<div class="trail-pin" style="left:${point.x / 4}%;top:${point.y / 3.6}%;--node-color:${DAY_PALETTE[i]};--marker-delay:${1.6 + i * .12}s"><div class="trail-location"><svg viewBox="-13 -17 26 34" aria-hidden="true"><path d="M0 14 C-3 10 -10 3 -10 -4 A10 10 0 1 1 10 -4 C10 3 3 10 0 14Z"/><circle cy="-4" r="3"/></svg></div></div>`).join("");
  const numbers = nodes.map((point, i) => `<div class="trail-marker" style="left:${point.x / 4}%;top:${point.y / 3.6}%;--node-color:${DAY_PALETTE[i]};--marker-delay:${1.6 + i * .12}s;--node-wave-delay:${2.8 + i}s;--lift:12px"><div class="trail-marker-lift"><div class="trail-number-bob"><svg class="trail-marker-art" viewBox="-12 -12 24 24" aria-hidden="true"><g class="trail-nodes"><g class="trail-number"><circle r="10"/><text y="4" text-anchor="middle">${i + 1}</text></g></g></svg></div></div></div>`).join("");
  const controls: [number, number, number, number][] = [[72, 240, 98, 182], [148, 178, 162, 230], [214, 222, 226, 222], [278, 202, 312, 186], [308, 130, 274, 106]];
