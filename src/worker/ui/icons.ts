...
/**
 * Google's G, the real one.
 *
 * `design/SignIn.dc.html` draws a dashed circle with a G in it and PLAN.md
 * section 5 says outright that it is a placeholder — Google ships the asset,
 * and its branding guidelines are the reason the button beside it reads
 * "Continue with Google" and not something friendlier. This is that asset:
 * four paths, four fixed colours, no stroke, so it takes no options.
 */
