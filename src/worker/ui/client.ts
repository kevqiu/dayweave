...
/**
 * Every clock inside a sentence, drawn the way the Settings screen asks:
 * "opens 11:00", "11:00 – 15:00, 17:30 – 22:00". The hours rules write 24-hour
 * clocks, as the rest of the data does, and are drawn through here.
 */
function clockText(text) {
  return String(text || "").replace(/\b\d{1,2}:\d{2}\b/g, (time) => displayTime(time));
}
...
function hoursBar(check, time) {
  ...
  return h("div", { class: "hours" }, [
    h("div", { class: "hours-head" }, [
      ...
      h("span", { class: "hours-label", text: clockText(check.label) }, []),
    ]),
    track,
    scale,
  ]);
...
function hoursFix(day, stop, check) {
  if (!check.closed && check.fix) {
    const fix = check.fix;
    return { label: "Move to " + displayTime(fix), run: () => setStopTime(stop, fix) };
  }
  const alt = nearestOpen(day, stop);
  if (!alt) return null;
  const target = dayById(alt.dayId);
  return {
    label: "Move to " + target.label + (alt.time ? " at " + displayTime(alt.time) : ""),
...
function hoursNotice(day, stop, check) {
  ...
  const title = check.closed
    ...
    : "Not open at " + displayTime(stop.time) + " on " + check.dayName;
  ...
  if (check.closed) {
    ...
    if (fix && fix.alt) {
      ...
      body = "The nearest day" + (city ? " in " + city : "") + " it is open"
        + (stop.time && !fix.alt.time ? " at " + displayTime(stop.time) : "")
        + " is " + fix.target.label + " (" + clockText(hours.label) + ").";
    } else {
...
function shutBands(stop, day, span) {
  ...
  for (const [a, b] of gaps) {
    ...
    const words = check.closed && open.length === 0
      ...
      : a === from && b < to ? "OPENS " + displayTime(PLAN.formatClock(b))
      : b === to ? "CLOSED FROM " + displayTime(PLAN.formatClock(a))
      : "CLOSED " + displayTime(PLAN.formatClock(a)) + " – " + displayTime(PLAN.formatClock(b));
  ...
  return h("button", {
    ...
    h("div", { class: "pick-text" }, [
      ...
      h("span", { class: "pick-why" }, [
        ...
        candidate.hours
          ? candidate.hoursWarn ? h("span", { class: "hours-flag", text: clockText(candidate.hours) }, []) : clockText(candidate.hours)
          : null,
      ]),
    ]),
...
function paintGridDrop() {
  ...
  if (when) when.textContent = displayTime(drag.gridTime) + (shut ? " · " + clockText(check.note) : "");
